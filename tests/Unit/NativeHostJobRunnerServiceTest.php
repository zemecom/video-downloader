...
PHP);
    ...
    public function testRunForVideoFhdInvokesCliWithQualityFhdFlag(): void
    {
        $root = \sys_get_temp_dir() . '/ytd_native_runner_video_fhd_' . \uniqid();
        \mkdir($root . '/bin', 0777, true);
        \mkdir($root . '/downloads', 0777, true);
        putenv('YTD_PROJECT_ROOT=' . $root);
...
PHP);
            ...
            $exitCode = $runner->run('job-123', 'https://example.com/watch?v=42', 'video-fhd');
            $argv = \json_decode((string) \file_get_contents($argvPath), true);

            self::assertSame(0, $exitCode);
            self::assertContains('--quality=fhd', $argv);
            self::assertContains('https://example.com/watch?v=42', $argv);
        } finally {
            putenv('YTD_PROJECT_ROOT');
        }
